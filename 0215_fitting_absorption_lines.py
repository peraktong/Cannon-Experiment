...
nor_ivar = np.ones((1,8575))
inf_flux = np.ones((1,8575))
parameters = np.array([[0,1,0,0,0,1,0,0]])
...
for i in range(0,N):
    ...
    ## flux,ivar,inf
    print(star_i[0].data.shape,star_i[1].data.shape,star_i[2].data.shape,star_i[4].data.shape)
    nor_flux = np.vstack((nor_flux,star_i[0].data))
    nor_ivar = np.vstack((nor_ivar,star_i[1].data))
    inf_flux = np.vstack((inf_flux,star_i[2].data))

    parameters = np.vstack((parameters,star_i[4].data))

N = len(parameters[:,0])
parameters = parameters[1:N,0:3]
nor_flux = nor_flux[1:N,:]
nor_ivar = nor_ivar[1:N,:]
inf_flux = inf_flux[1:N,:]

# new flux,ivar,inf,para
print(parameters.shape,nor_flux.shape,nor_ivar.shape,inf_flux.shape)
