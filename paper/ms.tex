...
To check whether it works, we can infer labels by using these normalized visit. If the inferred labels from each visit are similar, the normalization works. The deviations for each star are defined as the inferred labels for every visit minus the ones from the combined spectrum and we define them as $\triangle Teff$, $\triangle$ logg and $\triangle$ Fe/H. 

\begin{equation}
\triangle Teff = Teff_{inf}^{vs} - Teff_{inf}^{cb}
\end{equation}

$Teff_{inf}^{vs}$ and $Teff_{inf}^{cb}$ are the inferred labels for each visit and the combined spectrum.

If most of the deviations are small, the normalized spectrum for each visit doesn't lose shape, which means our normalization method works. The details are in experiment 5.
...
After that, we want to calibrate the spectrum for each visit. To begin with, we need to check whether our normalization works. The method is to infer labels for each visit and compare them. If our normalization works, the inferred labels from each visit for the same star should be similar to each other. We plot the histogram which shows the deviations of the inferred labels of each visits from the ones of combined spectrum for the same star. The deviations for each star are defined as the inferred labels for every visit minus the ones from the combined spectrum and we define them as $\triangle Teff$, $\triangle$ logg and $\triangle$ Fe/H. The data we use is the 614 stars in experiment 1 to 4. If most of the deviations are small, the normalized spectrum for each visit doesn't lose shape, which means our normalization method works.
