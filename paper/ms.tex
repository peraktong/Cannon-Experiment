...
We will do this optimization for each visit of every star in APOGEE DR10. The spectrum we use is in apstar file. We will re-analyze the velocities after the optimization and deliver back the velocities shift for the APOGEE data.
\bigskip
...
To begin with, we set the radial velocity to be positive if the star is leaving us. If a=1, b=0 and c=0, the optimized spectrum moves one pixel left from the inferred spectrum, which means the data spectrum should move one pixel right and the radial velocity of the data spectrum is not zero and it has a blue shift. Thus the radial velocity shift is negative and can be obtained by using (8). It's more convenient to use the logarithmic wavelength.
\begin{equation}
\log_{10}\lambda_0 - \log_{10}\lambda_s = \log_{10}(1+\beta)
\end{equation}

All spectrums in Apstar files are set to the same wavelength scale. The wavelength start at 15100.802 $\AA$ and the logarithmic wavelength grid spacing between two pixels is $6*10^{-6}$. By using (9), we find that one pixel is related to radial velocity 4144.68 m/s, which means if the data spectrum moves one pixel left, the radial velocity shift in the APOGEE data is $-4144.68 m/s$. 
\\ In the following formulas, $v_{shift}$ is the shift of the radial velocity in APOGEE data and $v_{pixel}$ =  4144.68 m/s, which is the radial velocity related to one pixel. $v_{light}$ is the speed of light.

We can use a, b and c to determine the radial velocity shift for the combined spectrum and each visit of the APOGEE data.
If the parameters are a, b and c, $a\over{a+b+c}$ of the spectrum moves one pixel left, $c\over{a+b+c}$ of the spectrum moves one pixel right and $b\over{a+b+c}$ of the spectrum doesn't move. The spectrum moves one pixel left has $v_{shift} = -v_{pixel}$ and the one moves one pixel right has $v_{shift} = v_{pixel}$. ${a+c}\over2$ of the spectrum is broadening but it's included in the spectrum moves left and right. Thus the data spectrum moves ${c-a}\over{(a+b+c)}$ pixel right and we have

\begin{equation}
\log_{10} (1+{v_{shift}}/v_{light}) = {{c-a}\over{(a+b+c)}}*\log_{10}(1+{v_{pixel}}/v_{light})
\end{equation}

$v_{shift}/v_{light}$ and $v_{pixel}/v_{light}$ are small so we can use Taylor expansion. Then we have:

\begin{equation}
v_{shift} = {{(c-a)*v_{pixel}}\over{a+b+c}}
\end{equation}
...
Here we plot the histogram of the radial velocity shifts in figure 11 and the radial velocity shifts in figure 12. The data we use is the combined spectrums of the 614 stars in the former experiment.

\begin{center}
\includegraphics[width=170mm]{figure_11.png}
\end{center}

\centerline{Fig.11 Histogram of the radial velocity shifts in combined spectrums for the 614 stars. }
\centerline{We exclude the outliers which have velocity shifts bigger than 2000 m/s}
\centerline{The formula to derive velocity shifts is $v_{shift} = {{(c-a)*v_{pixel}}\over{4\cdot(a+c-2b)}}$}
\bigskip
...
\centerline{Fig.12 Radial velocity shifts for combined spectrums of the 614 stars against mean fiber number}
\centerline{We exclude the outliers which have velocity shifts bigger than 2000 m/s}
\centerline{The formula to derive velocity shifts is $v_{shift} = {{(c-a)*v_{pixel}}\over{4\cdot(a+c-2b)}}$}
\bigskip

We also plot the four stars with biggest radial velocity shifts, which are in figure 13.

\begin{center}
\includegraphics[width=170mm]{figure_13.png}
\end{center}

\centerline{Fig.13 Spectrum of the four stars with biggest radial velocity shifts.}
...
\centerline{Fig.15 Histogram for Delta logg}
...
\centerline{Fig.16 Histogram for Delta Fe/H}
\bigskip
...
\begin{center}
\includegraphics[width=170mm]{figure_17.png}
\end{center}


\centerline{Fig.17 Comparison of each visit for the first star in figure 8.}
\bigskip
...
I plot the histogram of radial velocity shifts derived by using $v_{shift} = (c-a)*v_{pixel}$ in diagnostic figure 2 and velocity shifts against mean fiber number in diagnostic figure 3.

\begin{center}
\includegraphics[width=170mm]{d_figure_2.png}
\end{center}

\centerline{Diagnostic Fig.2 Histogram of radial velocity shifts}
\centerline{The formula to derive velocity shifts is $v_{shift} = (c-a)*v_{pixel}$}
\bigskip
...
\centerline{Diagnostic Fig.3 Radial velocity shifts against mean fiber number}
\centerline{The formula to derive velocity shifts is $v_{shift} = (c-a)*v_{pixel}$}
\bigskip


We also plot the four stars with biggest radial velocity shifts, which are in figure 13. The formula I use is $v_{shift} = (c-a)*v_{pixel}$.

\begin{center}
\includegraphics[width=170mm]{d_figure_4.png}
\end{center}

\centerline{Diagnostic Fig.4 Spectrum of the four stars with biggest radial velocity shifts.}

\centerline{ The black line is the observed spectrum. The green line is the inferred spectrum and the red line is the optimized spectrum.}

\bigskip

The original histogram of radial velocity shifts for figure 11.

\begin{center}
\includegraphics[width=170mm]{d_figure_5.png}
\end{center}

\centerline{Diagnostic Fig.5 Histogram of radial velocity shifts}
\centerline{The formula to derive velocity shifts is $v_{shift} = {{(c-a)*v_{pixel}}\over{4\cdot(a+c-2b)}}$}
\bigskip

\begin{center}
\includegraphics[width=170mm]{d_figure_6.png}
\end{center}

\centerline{Diagnostic Fig.6 Radial velocity shifts against mean fiber number}
\centerline{The formula to derive velocity shifts is $v_{shift} = {{(c-a)*v_{pixel}}\over{4\cdot(a+c-2b)}}$}
\bigskip


To check which method is better, we choose the four stars with smallest abs(a-c). These four stars have abs(a-c) smaller than 0.001 and we think their radial velocity shifts are 0. We call $v_{shift} = {{(c-a)*v_{pixel}}\over{4\cdot(a+c-2b)}}$ method A and $v_{shift} = (c-a)*v_{pixel}$ method B.

First, move the data spectrum of the four stars one pixel left and the radial velocity shift should be $-v_{pixel}$= -4144.68$m/s$



\begin{center}
\includegraphics[width=170mm]{d_figure_7.png}
\end{center}

\centerline{Diagnostic Fig.7 Move the data spectrum one pixel left.}
\centerline{the radial velocity shift should be $-v_{pixel}$= -4144.68$m/s$}
\centerline{ The black line is the observed spectrum. The green line is the inferred spectrum and the red line is the optimized spectrum.}
\bigskip
...
Then, move the data spectrum of the four stars one pixel right and the radial velocity shift should be $v_{pixel}$= 4144.68$m/s$


\begin{center}
\includegraphics[width=170mm]{d_figure_8.png}
\end{center}

\centerline{Diagnostic Fig.8 Move the data spectrum one pixel right.}
\centerline{the radial velocity shift should be $v_{pixel}$= 4144.68$m/s$}
\centerline{ The black line is the observed spectrum. The green line is the inferred spectrum and the red line is the optimized spectrum.}
