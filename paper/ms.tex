...
$\sigma_{jn}^2$ is the data uncertainty for star n at pixel j. The number of stellar and pixel each star are N and J. Here N=548 and J = 8575. Write the objective function in the form of matrix can make it simpler. So we introduce several matrix: Y, A, C and X, which are defined as followed:
...
The result is \(a+b+c = 0.99990\). 

(a,b,c) = (-0.00215, 1.00462, -0.00258 ).
...
%It works!
 ...
 Here a,b and c are the parameters obtained from spectrum optimization. Then we infer stellar labels by using $\theta_{j}^{opt}$ and generate optimized inferred labels. The data we use is the training set, which includes 548 stellar with 8575 pixel each.
...
%% Third paragraph
 ...
 Choose four stellar, which are not in the training set: 
...
%\includegraphics[width=170mm]{figure_2.png}
 ...
 The optimized spectrum has a 1\% smaller chi-squared so our optimization works.
  
 \subsection{\label{sec:level2}Experiment 2}
 Optimize $\theta_j$ and generate optimized inferred labels. Then plot the inferred labels and the optimized inferred labels.
 \bigskip
 
 %\includegraphics[width=170mm]{figure_3.png}
 \centerline{Fig.3 Inferred labels and the reference labels}
 \bigskip
 
 Finally compare the chi-square.
 
The chi-squared becomes 1\% bigger after the optimization. We need to do bigger optimization before inferring the labels!
