...
In a word, the method we use to calibrate the radial velocities of the combined spectra and individual visits is:


\begin{description}
 \item[1] Infer labels from the fluxes of the combined spectra and individual visits. 
 ...
 \item[4] By assuming these inferred fluxes are at rest, derive the radial velocity shifts of these combined spectra and individual visits.
\end{description}
...
We will show our results are better than APOGEE's in the nect experiment.


\subsection{\label{sec:level2}Experiment 3}

Finally, we shows our RV shifts results are better than results from the APOGEE team. We demonstrate it by two steps:


\begin{description}
  \item[1] Use inferred fluxes as templates are reliable. These inferred fluxes are at rest.
  \item[2] Our method for deriving the relative radial velocities between inferred fluxes and data spectra is good.

\end{description}



First, we show that use inferred fluxes as templates are reliable. Here we randomly choose 5 stars with different SNR, Teff, logg and Fe/H. Then plot the fluxes from the APOGEE team and the Cannon.
...
From these plots, we can see that the peak locations of fluxes from the APOGEE team shift, which should not happen because these fluxes are set at rest by the APOGEE team and there shouldn't be shifts between each fluxes. This means the radial velocities from the APOGEE team have some errors. When we look at the fluxes generated from the Cannon, the peaks from individual visits have the same positions. That's to say, we can use inferred fluxes from the Cannon as templates for calibration.
\bigskip

Then, we argue that our method for deriving the relative radial velocities between inferred fluxes and data spectra is good.
