...
def plot_visit_sim_old_customized(name):
    ...
    for i in range(0, N):
        ...
        if mask[i]==1:
            ...
            data_label = "Data flux"
            plt.plot(0,0,"bo",label="Data flux of the visit with 2b < a+c")
            color = "red"
        ...
        # inferred flux
        plt.plot(wl, inf_flux[i,:], "g", label="Inferred flux  Teff=%.2f K logg=%.2f Fe/H =%.2f chi-squared = %.2f" % (
...
onlyfiles = onlyfiles[0:len(onlyfiles)]
print(onlyfiles)
print(len(onlyfiles))
